      ...
      <div className="bg-white rounded-2xl border-2 border-indigo-200 shadow-md p-6 space-y-6">
        ...
        <details className="group">
          ...
          <div className="mt-4 space-y-4 pl-1">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => set('verbalization', !s.verbalization)}
                className={`w-10 h-6 rounded-full transition-colors relative ${s.verbalization ? 'bg-indigo-600' : 'bg-gray-300'}`}
              >
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${s.verbalization ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </button>
              <span className="text-sm text-gray-700">🗣️ Verbalisation — rappel à voix haute avant validation</span>
            </div>

            <div className="flex items-center gap-3">
              <button
                onClick={() => set('skipPlanning', !s.skipPlanning)}
                className={`w-10 h-6 rounded-full transition-colors relative ${s.skipPlanning ? 'bg-violet-600' : 'bg-gray-300'}`}
              >
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${s.skipPlanning ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </button>
              <span className="text-sm text-gray-700">⚡ Mode exécution directe — sans phase de planification</span>
            </div>
          </div>
        </details>
